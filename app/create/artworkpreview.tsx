...
export default function ArtworkPreview({
  ...
  // Calculate projection that preserves aspect ratio
  ...
  // Convert geo to SVG coordinates using proper projection
  ...
  // Generate humanized path segment (matches main page logic)
  ...
  // Store anchor positions for reference
  anchors.forEach((anchor) => {
    const pos = geoToSVG(anchor.lng, anchor.lat);
  ...
  return (
    <div className="fixed inset-0 bg-neutral-900 bg-opacity-95 z-50 flex items-center justify-center p-8">
      ...
      {/* Preview container */}
      <div className="w-full max-w-4xl bg-white shadow-2xl" style={{ aspectRatio: "420/594" }}>
        {/* Loading indicator */}
        {isLoadingGeo && (
        ...
        >
          ...
          {/* Geographic features - layered for proper visual hierarchy */}
          
          {/* Layer 1: Bathymetry contours (ocean depth, very subtle) */}
          {!isLoadingGeo && geoData?.bathymetry && geoData.bathymetry.length > 0 && (
            <g opacity="0.08" stroke="#7B9CAA" strokeWidth="0.25" fill="none">
              {geoData.bathymetry.map((contour: any, i: number) => {
                const pathData = contourToSVGPath(contour, projection);
                if (!pathData) return null;
                return <path key={`bathy-${i}`} d={pathData} />;
              })}
            </g>
          )}
          
          {/* Layer 2: Lakes (filled polygons, light tint) */}
          {!isLoadingGeo && geoData?.lakes && geoData.lakes.features.length > 0 && (
          ...
          {/* Layer 3: Elevation contours (hand-drawn feel) */}
          {!isLoadingGeo && geoData?.contours && geoData.contours.length > 0 && (
            <g opacity="0.18" stroke="#8B7D6B" strokeWidth="0.4" fill="none" strokeLinecap="round" strokeLinejoin="round">
              {geoData.contours.map((contour: any, i: number) => {
                const pathData = contourToSVGPath(contour, projection);
                if (!pathData) return null;
                // Vary opacity by elevation (higher elevations slightly more visible)
          ...
          {/* Layer 4: Rivers (thin lines, subtle blue-gray) */}
          {!isLoadingGeo && geoData?.rivers && geoData.rivers.features.length > 0 && (
            <g opacity="0.22" stroke="#7B9CAA" strokeWidth="0.35" fill="none" strokeLinecap="round">
          ...
          {/* Layer 5: Coastlines (medium visibility) */}
          {!isLoadingGeo && geoData?.coastlines && geoData.coastlines.features.length > 0 && (
            <g opacity="0.28" stroke="#8B7D6B" strokeWidth="0.55" fill="none" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Layer 6: Administrative borders (dashed, subtle) */}
          {!isLoadingGeo && geoData?.borders && geoData.borders.features.length > 0 && (
            <g opacity="0.15" stroke="#A39B8F" strokeWidth="0.3" fill="none" strokeDasharray="2,1" strokeLinecap="round">
          ...
          {/* Fallback: subtle grid if geo data not fully loaded */}
          {(isLoadingGeo || (!geoData?.coastlines?.features?.length && !geoData?.contours?.length)) && (
          ...
          {/* Main path - rendered prominently over geography */}
          <path
            d={fullPath}
            stroke="#8B7D6B"
            strokeWidth="0.75"
